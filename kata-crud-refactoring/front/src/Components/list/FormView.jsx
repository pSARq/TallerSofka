...
const FormView = () => {
  ...
  //Agrega una nueva lista de tareas
  const onCreate = (event) => {
    ...
    ApiList.save({ name: state.name, id: null }).then((response) => {
      if (response.ok) {
  ...
  //Muestra la entrada de texto de la lista de tareas y el boton de nueva lista
  return (
    <form ref={formRef}>
      <input
        type="text"
      ...
      {requiredName && (
        <p className="text-danger">
          Debe agregarle un nombre a la lista de tareas
        </p>
      )}
    </form>
  );
